...
//        this.dumpBoard();
    ...
    onConnect: function(socket) {
        ...
        if (socket && socket.id && !this.allConnections[socket.id]) {
            ...
            this.allConnections[socket.id] = {      // **                     connData                      **
                ...
                dirNext: -1,                        // direction client has requested (-1 means "no request")
                zJunkLastItem: null
            };
            this.nConnections++;
    ...
    onClientEvent: function(evt, data, connData, socket) {
        ...
        var fn = "on"+evt;
        if (this[fn]) {
            this[fn](evt,data, connData, socket);
        }
    },

    // client is "Ready"
    onIAmReady: function(evt,data, connData, socket) {
        connData.isReady = true;            // mark this client as "Ready"
        this.broadcast_connections();
    },

    // client wants to change direction
    onKeyStroke: function(evt,data, connData, socket) {
        switch (data.dir) {
            case "right":
                connData.dirNext = 0;
                break;
